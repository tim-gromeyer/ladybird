...
FFmpegIOContext::FFmpegIOContext(NonnullRefPtr<ByteStreamCursor> stream_cursor, AVIOContext* avio_context)
...
ErrorOr<NonnullOwnPtr<FFmpegIOContext>> FFmpegIOContext::create(NonnullRefPtr<ByteStreamCursor> stream_cursor)
{
    ...
    auto* avio_context = avio_alloc_context(
        ...
        [](void* opaque, u8* buffer, int size) -> int {
            auto& stream_cursor = *static_cast<ByteStreamCursor*>(opaque);
        ...
        [](void* opaque, int64_t offset, int whence) -> int64_t {
            ...
            auto seek_mode_from_whence = [](int origin) -> ByteStreamCursor::SeekMode {
                if (origin == SEEK_CUR)
                    return ByteStreamCursor::SeekMode::FromCurrentPosition;
                if (origin == SEEK_END)
                    return ByteStreamCursor::SeekMode::FromEndPosition;
                return ByteStreamCursor::SeekMode::SetPosition;
            };
